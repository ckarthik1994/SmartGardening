...
void loop() {
  if (received == true) {
    ...
    if (fail) {printf("Decoding error.\n");}
      ...
      std::stringstream s;
      s << "{ \"datetime\": \"" << getCurrentDateTime() << "\", \"temperature\": " << (int)(((temp-1024)/10+1.9+0.5)*9/5+32) << ", \"humidity\": " << humidity << " }";
      std::ofstream out("data/sensor_data_"+getCurrentDateTime2()+".json");
      out << s.str();
      out.close();
